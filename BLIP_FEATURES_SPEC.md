...
- KYC: users must provide name + phone + address to place orders; CNIC/ID upload + admin verification workflow are implemented.
...
- Order status notifications + receipts (email/SMS/push; provider plan: SendGrid email + SMS4Connect SMS for Pakistan coverage).
